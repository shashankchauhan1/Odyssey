      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-gradient-to-br from-indigo-500 to-indigo-600 p-6 rounded-2xl shadow-lg text-white transform hover:scale-105 transition-transform duration-200">
          <div className="flex items-center justify-between mb-2">
            <p className="text-indigo-100 text-sm font-semibold uppercase tracking-wide">Total Budget</p>
            <Wallet className="w-5 h-5 text-indigo-200" />
          </div>
          <p className="text-3xl font-extrabold">₹{budget.toLocaleString()}</p>
        </div>
        ...
        <div className="bg-gradient-to-br from-purple-500 to-purple-600 p-6 rounded-2xl shadow-lg text-white transform hover:scale-105 transition-transform duration-200">
          <div className="flex items-center justify-between mb-2">
            <p className="text-purple-100 text-sm font-semibold uppercase tracking-wide">Total Spent</p>
        ...
        <div className={`bg-gradient-to-br p-6 rounded-2xl shadow-lg text-white transform hover:scale-105 transition-transform duration-200 ${isOverBudget ? 'from-red-500 to-red-600' : 'from-emerald-500 to-emerald-600'}`}>
          <div className="flex items-center justify-between mb-2">
            <p className={`text-sm font-semibold uppercase tracking-wide ${isOverBudget ? 'text-red-100' : 'text-emerald-100'}`}>
              {isOverBudget ? 'Over Budget!' : 'Remaining'}
            </p>
            <AlertCircle className={`w-5 h-5 ${isOverBudget ? 'text-red-200' : 'text-emerald-200'}`} />
          </div>
          <p className="text-3xl font-extrabold">
            {isOverBudget && '-'}₹{Math.abs(remaining).toLocaleString()}
          </p>
          {isOverBudget && (
            <p className="text-xs text-red-100 mt-2">Consider adjusting your spending</p>
          )}
        </div>
      </div>
              ...
              {trip.expenses?.slice().reverse().map((expense, i) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    <span className="font-bold text-gray-900 text-lg">-₹{expense.amount.toLocaleString()}</span>
                    <button
